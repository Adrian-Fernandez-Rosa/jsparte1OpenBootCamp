// Cómo trabajar con listas (arrays, arreglos, vectores...)


let array = [1, 2, 3, 4, 5, 6];

// Acceder a los valores de un array a través de su posición.

console.log(array[0]);

// Métodos para introducir nuevos valores en nuestros arrays
// .push() .unshift => Mutan el valor de nuestro array
// Valores al final -> Push
array.push();

array.push("final");
console.log(array);

array.unshift("inicio");
console.log(array);

// Métodos para eliminar valores de arrays
// .pop() .shift() => Mutan el valor del array

const array2 = [1, 3, "hola", false];
// Valores al final -> Pop.

array2.pop();
console.log(array2);

// Valores al principio -> Shift

array2.shift()
console.log(array2);

// Métodos para eliminar, modificar o añadir valores en nuestro arrays
// .splice(x, y, z)
const array3 = [1, 2, 3, 4, 5, 6];

// Eliminar valores
array3.splice(2, 3); //primer parametro: indice desde donde eliminar,
// segundo parametro cuantos 
console.log(array3);

// Añadir valores .splice(indice, 0, valores)

array3.splice(2, 0, "hola");
console.log(array3);

// Modificar valores .splice(indice, 1, valores)

array3.splice(2, 1, 3);
console.log(array3);